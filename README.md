...
**앱 주소: https://ops.bonnoel.com** (Vercel. main에 push하면 1분 안에 자동 반영. 예전 주소 https://bonnoel2014.github.io/bonnoel-ops/ 도 같이 살아있음) 홈 화면 하나에 아이콘별로 도구가 붙습니다.
...
매장폰 선결제 바로가기: 주소 뒤에 `#/prepay`를 붙여 열면(`https://ops.bonnoel.com/#/prepay`), 세션이 없을 때만 이름·비밀번호를 한 번 물어보고 **홈 화면을 거치지 않고 바로 선결제 화면**으로 가요. 매장도 `#/clock`처럼 처음 한 번 고르면 그 폰에 기억돼서 다음부터는 안 물어봐요(이름·비밀번호는 매번 확인 — 누가 처리했는지 기록에 남기기 위해서라 일부러 생략 안 함). 이 주소를 홈 화면에 추가해 두면 돼요.
...
재직·경력증명서(직원 신청 → 사장님 승인 → PDF 메일), 주주명부(기준일 골라 PDF), 법인 서류함(등본·사업자등록증 보관 + 기한 링크), 발급 대장(발급번호 + QR 진위 확인). 주주·서류함은 선결제처럼 잠겨 있고 **선결제 사장님 코드(6자리)** 로만 열려요. 기획: [`기획/법인서류_기획안.md`](기획/법인서류_기획안.md)
...
주소 뒤에 `?demo=1`을 붙이면 데이터 창고 없이 가짜 데이터로 눌러볼 수 있습니다. (비밀번호: 사장님 0000 · 매니저 1111 · 알바 F 2222 · 알바 E는 처음 / 선결제 코드: 사장님 000000 · 성수 111111 · 왕십리 222222 · 중계 333333 · 답십리 444444 / `prepay.html?demo=1` 손님 조회 비밀번호 1234 / 법인 서류 코드 000000 — 가짜 회사 "데모베이커리")

## 파일

- `index.html` — 앱 전체 (HTML + CSS + JS 한 파일)
- `docs-templates.js` — 입사 서류 양식, `corp-templates.js` — 재직·경력증명서·주주명부 양식
