...
규칙: 앱에 없는 이름은 새 직원으로 추가(체크 해제하면 건너뜀). 퇴사일 칸에 진짜 날짜가 있으면 "퇴사 처리 제안"에 뜨고, 체크 해제하면 그 사람은 그대로 둬요. 날짜가 아닌 글자("왕십리이동" 같은 지점 이동 메모 등)는 자동으로 퇴사 처리하지 않고 "확인 필요"로만 보여줘요 — 진짜 퇴사면 **직원·비밀번호**에서 직접 퇴사 버튼을 눌러 주세요. 주민번호·주소·전화·이메일은 어디에도 저장하지 않고, 지점·이름·입사일·보건증 만료일·퇴사일만 씁니다. **직원·비밀번호** 화면은 이제 퇴사자를 기본으로 접어 두고, 근무자만 먼저 보여줘요.
